...
public class ConcertSeatArrangementStrategy implements SeatArrangementStrategy {
    ...
    private void arrangeSeatingArea(Event event) {
        ...
        for (int i = 1; i <= 100; i++) {  // Let's assume 100 seats in the seating area
            event.getSeats().add(new Seat("S" + i, SeatType.GOLD, true, 100.0, "Seating", EventType.CONCERT, event));
        }
    }
    ...
    private void arrangeStandingArea(Event event) {
        ...
        for (int i = 1; i <= 200; i++) {
            event.getSeats().add(new Seat("ST" + i, SeatType.PLATINUM, true, 30.0, "Standing",  EventType.CONCERT, event));
        }
    }
    ...
    private void arrangeLawnSeats(Event event) {
        ...
        for (int i = 1; i <= nearStageCount; i++) {
            event.getSeats().add(new Seat("ST-NearStage-" + i, SeatType.PLATINUM, true, 50.0, "Near Stage",  EventType.CONCERT, event));
        }
        ...
        for (int i = 1; i <= middleBufferCount; i++) {
            event.getSeats().add(new Seat("ST-MiddleBuffer-" + i, SeatType.GOLD, true, 40.0, "Middle Buffer",  EventType.CONCERT, event));
        }

        // 3. Back Area
        int backAreaCount = 50;  // Example: 50 standing spots at the back
        for (int i = 1; i <= backAreaCount; i++) {
            event.getSeats().add(new Seat("ST-Back-" + i, SeatType.SILVER, true, 30.0, "Back",  EventType.CONCERT, event));
        }
    }
